...
def train_or_eval_cm(epoch, feature, label2emotion, label2id, dataset, role, cicero, comet, tokenizer, model,
        ...
        for data in dataloader:
            ...
            for vid in vids:
                ...
                for i in range(length - 1):
                    ...
                    listener = role[vid][i+1]
                    print("speaker:", listener, "emotion:", label2emotion[cur_emotions[i]])
                    cur_com = torch.tensor([cur_comet[j] for j in range(9)])
                    cur_feature = find_tab_text_feature(listener, feature, tokenizer, U.shape[1])
                    com_r = torch.as_tensor(cicero[cicero_id], dtype=torch.float32).cuda()
                    cicero_id = cicero_id + 1
                    continue
                    common_ground = get_common_ground(i, U, cur_com, role[vid], feature, tokenizer)
                    target = "Target:" + sent
                    ...
                    label = label.cuda()
                    _, loss, logits = model(U, common_ground, com_r, role[vid], cur_feature, i, input_ids=input_ids,
            ...
            for data in dataloader:
                ...
                for vid in vids:
                    ...
                    for i in range(length - 1):
                        ...
                        cur_emotion = cur_emotions[i + 1]
                        cur_emotion = label2emotion[cur_emotion]
                        listener = role[vid - 1153][i + 1]
                        cur_com = torch.tensor([cur_comet[j] for j in range(9)])
                        cur_feature = find_tab_text_feature(listener, feature, tokenizer, U.shape[1])
                        com_r = torch.as_tensor(cicero[cicero_id], dtype=torch.float32).cuda()
                        cicero_id = cicero_id + 1
                        common_ground = get_common_ground(i, U, cur_com, role[vid - 1153], feature, tokenizer)
                        target = "Target:" + sent
                        fix_prompt = "The possible emotional reaction of the " + listener + " in response to target is "
                        x = ' <mask>' * (ple + plp + 2) + ' ' + target + ' ' + fix_prompt + ' <mask>' * (1 + pre + prp)
                        y = ' <mask>' * (
                        ...
                        ground_truth.append(cur_emotions[i + 1])
                        label = label.cuda()
                        _, loss, logits = model(U, common_ground, com_r, role[vid - 1153], cur_feature, i, input_ids=input_ids,
                                                attention_mask=attention_mask, labels=label)
                        logits = logits[:, - ple - plp - 2]
                        logits = logits.data.cpu()
                        pred = int(logits.argmax(-1))
                        predict[vid] = (label2id[pred] if pred in label2id.keys() else 0)
                        preds.append(label2id[pred] if pred in label2id.keys() else 0)
...
def cm_prompt_feature_TP(path, label2emotion, label2id, ple, plp, pre, prp, epoch, batch_size, lr, l2):

    tokenizer = RobertaTokenizer.from_pretrained('./roberta_large_tokenizer/')
    model = CMPromptFIX.from_pretrained('./roberta_large_tokenizer/')
    torch.cuda.set_device(0)
    dataset = MELDDataset(path=path)
    role = roleDataset()
    role_test = role_testDataset()
    comet = MELDComet('./Datasets/MELD/meld_features_comet.pkl')
    cicero = cicero_get(tokenizer, train=True)
    cicero_test = cicero_get(tokenizer, train=False)
    train_loader, valid_loader, test_loader = get_proper_loaders(path, batch_size=batch_size)
    model.cuda()
    optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=l2)
    best_fscore = None
    best_epoch = 0
    train_feature = get_tab_text_feature(train=True)
    test_feature = get_tab_text_feature(train=False)
    fscore_list = []
    for e in range(epoch):
        train_results, train_losses, _, _ = train_or_eval_cm(e, train_feature, label2emotion, label2id,
                                                                                    dataset, role, cicero, comet, tokenizer,
                                                                                    model, optimizer, train_loader,
                                                                                    ple=ple, plp=plp, pre=pre, prp=prp,
                                                                                    train=True)

        test_results, test_losses, f_score, pred = train_or_eval_cm(e, test_feature, label2emotion,
